...
func (s *CleanupService) RunCleanup(ctx context.Context) (*CleanupResult, error) {
	...
	absenceCutoff := time.Now().AddDate(0, 0, -s.retentionDays*3)
	r, err = s.db.ExecContext(ctx, `DELETE FROM absences WHERE end_date < $1`, absenceCutoff)
	if err == nil {
		if rows, _ := r.RowsAffected(); rows > 0 {
			result.AbsencesDeleted = rows
		}
	}
